...
/// <summary>
/// Reusable chat surface combining a native <see cref="ChatShell"/> chrome with an embedded
/// WebView2 that hosts the gateway-served chat UI. Handles CSS chrome-hiding, native composer
/// relay (text + dropdowns), and error/placeholder states. Used by both the tray popup
/// (ChatWindow) and the Hub Chat tab (ChatPage) so the chat UX stays in one place.
/// </summary>
public sealed partial class ChatSurface : UserControl
{
    ...
    /// <summary>
    /// Initialize the embedded WebView2 with the given gateway URL and token. Idempotent — safe
    /// to call multiple times; subsequent calls with different parameters trigger a re-navigate.
    /// </summary>
    public void Initialize(string gatewayUrl, string token)
    {
        if (string.IsNullOrEmpty(gatewayUrl)) return;

        if (_webViewInitialized)
        {
            // Already initialized — re-navigate if URL/token changed.
            if (_gatewayUrl != gatewayUrl || _token != token)
            {
                _gatewayUrl = gatewayUrl;
                _token = token;
                if (TryBuildChatUrl(gatewayUrl, token, out var newUrl, out _))
                {
                    _chatUrl = newUrl;
    ...
    private async Task InitializeWebViewAsync()
    {
        try
        {
            if (!TryBuildChatUrl(_gatewayUrl, _token, out var chatUrl, out var errorMessage))
            {
            ...
            // Inject CSS to hide gateway web chrome and reflow layout into a flexbox column so the
            // chat thread fills the available height. Runs before any page script via
            // AddScriptToExecuteOnDocumentCreatedAsync so React/Lit components see it from first paint.
    ...
    /// <summary>Re-navigate to the chat home URL.</summary>
    ...
    /// <summary>Reload the current WebView2 document.</summary>
    public void Reload()
    {
        if (_webViewInitialized)
    ...
    /// <summary>Open the chat URL in the system default browser.</summary>
    ...
    /// <summary>Open Chromium DevTools for the embedded WebView2.</summary>
    ...
    private void OnOpenInBrowserClick(object sender, RoutedEventArgs e) => OpenInBrowser();
    ...
    private async void OnShellSendRequested(object? sender, string text)
    {
        ...
        try
        {
            ...
            Debug.WriteLine($"[ChatSurface] SendRequested result: {result}");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[ChatSurface] SendRequested failed: {ex.Message}");
        }
    }
    ...
    private async void OnShellDropdownChanged(object? sender, (string Kind, string Value) e)
    {
        ...
        {
            Debug.WriteLine($"[ChatSurface] Dropdown {e.Kind} failed: {ex.Message}");
        }
    }
    ...
    private async Task PopulateDropdownsAsync()
    {
        ...
        for (int attempt = 0; attempt < 20; attempt++)
        {
            ...
            {
                Debug.WriteLine($"[ChatSurface] PopulateDropdowns attempt {attempt} failed: {ex.Message}");
            }
        }
    }
